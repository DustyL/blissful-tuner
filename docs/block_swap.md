...
## blissful-tuner: status & runtime-gating (H2D-only)

H2D-only block swap was ported from upstream Musubi Tuner (commit `9bfc5a4`). The
mechanical integration — `BlockSwapConfig` + `create_offloader()` factory, the
`enable_block_swap(blocks_to_swap, config)` signature across every architecture, the
training/inference call sites, and the `--gradient_checkpointing` guard — is wired and
covered by `tests/test_h2d_block_swap.py` (config/factory guards + a CUDA forward/backward
**parity** smoke including the re-prepare round-trip).

**Before enabling `--block_swap_h2d_only` in a production run, validate these on your own
hardware (they are untested upstream against this fork's specifics and have bitten the
block-swap path before — see CLAUDE.md "Block Swap Invariants"):**

1. **`torch.compile` (`--compile`).** The classic offloader swaps `weight.data` (storage);
   H2D-only swaps the `module.weight` **Parameter object** itself each ring rotation. Under
   block-level compilation this can trigger guard failures / recompiles or a stale captured
   weight. Test compiled-vs-eager loss parity with no per-step recompilation.
2. **`--fp8_scaled` on the `torch._scaled_mm` path.** The ring weight is a `view` into an
   `align=256` flat byte buffer; `_scaled_mm` has strict float8 layout/contiguity
   requirements the classic contiguous allocation satisfied. Most likely concrete failure.
3. **Prior preservation (mask loss + EMA/base teacher).** The teacher no_grad forward fires
   a re-prepare **every step** (`restore_block_swap_after_no_grad_forward`), which defeats
   the offloader's "ring survives the forward↔backward turnaround" optimization — each step
   pays a cold-start H2D of `ring_size` blocks. Confirm the H2D-only throughput win still
   holds against the classic `ModelOffloader` baseline under your prior-preservation recipe.

Until 1–3 pass on the RTX 5090, prefer classic block swap (`--blocks_to_swap` without
`--block_swap_h2d_only`) for FLUX.2 Klein + DoRA + compile + fp8_scaled production training.
