...
def parse_args() -> argparse.Namespace:
    ...
    # fp8 + fused attention is incompatible: under --fp8_scaled the DiT presents fp32 activations
    # to attention, which the fused flash / xformers / sageattn kernels reject (verified: both raise
    # "only support fp16/bf16"). Only torch (SDPA) accepts fp32. Fail fast with the fix rather than
    # a confusing mid-run kernel error. Use --attn_mode torch with fp8, or a fused backend in bf16.
    if args.fp8_scaled and args.attn_mode != "torch":
        raise ValueError(
            f"--fp8_scaled is only compatible with --attn_mode torch (got '{args.attn_mode}'). "
            "Under fp8 the DiT feeds fp32 activations to attention, which the fused flash/xformers/"
            "sageattn kernels reject (only SDPA/torch accepts fp32). Use '--attn_mode torch' together "
            "with --fp8_scaled, or drop --fp8_scaled to run a fused backend in bf16."
        )
