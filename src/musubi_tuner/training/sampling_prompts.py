...
def should_sample_on_step(args, step: int) -> bool:
    """Check whether to sample based on step count only (blissful trigger-gated sampling)."""
    if step == 0:
...
def should_sample_on_epoch(args, epoch: int) -> bool:
    """Check whether to sample based on epoch count only (blissful trigger-gated sampling)."""
    if epoch == 0:
        return getattr(args, "sample_at_first", False)
