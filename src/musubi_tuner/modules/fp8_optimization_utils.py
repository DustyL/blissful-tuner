...
def fp8_linear_forward_patch(self: nn.Linear, x, use_scaled_mm=False, max_value=None):
    ...
    if use_scaled_mm and (
        ...
        out_dtype = (
            input_dtype if input_dtype != torch.float32 else original_weight_dtype
        )  # This guard is needed for HyVideo somewhy
...
def apply_fp8_monkey_patch(model, optimized_state_dict, use_scaled_mm=False, exclude_ffn_from_scaled_mm=False):
    ...
    Args:
        ...
        use_scaled_mm (bool): Use scaled_mm for FP8 Linear layers, requires SM 8.9+ (RTX 40 series)
        exclude_ffn_from_scaled_mm (bool): If enabled, do not use fp8 math on feedforwards when use_scaled_mm. Beneficial for Wan

    Returns:
        nn.Module: The patched model (same instance, modified in-place)
    """
    if use_scaled_mm and exclude_ffn_from_scaled_mm:
