...
class ImageDataset(BaseDataset):
            ...
            while len(futures) >= num_workers or (consume_all and len(futures) > 0):
                ...
                for future in completed_futures:
                    ...
                    if controls is not None:
                        ...
                        # Add every control size to bucket_reso so that different control resolutions AND
                        # a different number of control images go to different batches. Run this whenever
                        # control data is present (not only for no_resize_control / control_resolution):
                        # otherwise items with a different control count share a batch and the collator
                        # ragged-stacks latents_control_{i} (crash) or silently misaligns controls between
                        # samples. controls is already in index order, so the appended shapes stay
                        # index-aligned.
    ...
    def prepare_for_training(self, num_timestep_buckets: Optional[int] = None):
        ...
        for cache_file in latent_cache_files:
            ...
            # which strip_dtype_suffix keeps in each element) is deterministic across cache files.
            ...
            if control_keys:
                ...
                control_shapes = [strip_dtype_suffix(key) for key in control_keys]
