# Vendored verbatim from canonical src/ideogram4/scheduler.py (logit-normal schedule + v4 sampler presets).
...
class SamplerParameters:
    ...
    def __post_init__(self) -> None:
        if len(self.guidance_schedule) != self.num_steps:
            raise ValueError(f"guidance_schedule has length {len(self.guidance_schedule)}, expected num_steps={self.num_steps}")
