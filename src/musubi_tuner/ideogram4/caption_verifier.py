# Vendored verbatim from the canonical Ideogram 4 repo (src/ideogram4/caption_verifier.py).
# Pure-stdlib structured-JSON caption checker. verify_raw() RETURNS a list of warning strings
# (empty = OK) and never raises — the warn-only / strict policy lives in verify_caption() below.
...
NON_ASCII_UNICODE_ESCAPE_RE = re.compile(r"\\u(?:00[89a-fA-F][0-9a-fA-F]|0[1-9a-fA-F][0-9a-fA-F]{2}|[1-9a-fA-F][0-9a-fA-F]{3})")
...
class CaptionVerifier:
    ...
    def _verify_style_description(self, sd, warnings: list[str]) -> None:
        ...
        if has_photo and has_art_style:
            ...
            warnings.append(
                "style_description: expected one of 'photo' (for photo captions) or 'art_style' (for non-photo captions)"
            )
            return
    ...
    def _verify_compositional_deconstruction(self, cd, warnings: list[str]) -> None:
        ...
        if "background" not in cd:
            ...
            warnings.append(f"compositional_deconstruction.background: expected a string, got {type(cd['background']).__name__}")
            return
...
# --- Blissful warn-only policy wrapper (H4) -------------------------------------------------------
# The canonical verifier above only RETURNS warnings. Blissful's plain-.txt LoRA workflows must not be
# bricked by structured-JSON expectations, so verify_caption() is WARN-ONLY by default; strict raising is
# an explicit opt-in. Used by ideogram4_cache_text_encoder_outputs (default warn-only).
