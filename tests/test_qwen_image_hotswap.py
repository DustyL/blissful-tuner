"""Tier 1 #1c Qwen-Image hotswap regression tests.

Pins the Qwen-Image generalization of the compile-friendly LoRA hotswap design
(see docs/plans/2026-05-04-peft-tier1c-zimage-qwenimage-hotswap.md):
  ...
  - prepare_qwen_image_hotswap_state() sets model.hotswap_state to None when off
  - prepare_qwen_image_hotswap_state() captures un-merged base when on
  - note_qwen_image_initial_loras() records the initial active LoRA set
  - note_qwen_image_initial_loras() pads multipliers to 1.0 (matches WAN convention)
...
Single hotswap implementation works for all 5 Qwen-Image variants
(original / edit / edit-2509 / edit-2511 / layered) since they share
QwenImageTransformerBlock and load_qwen_image_model.
"""
...
def _save_tiny_dit(dirpath: Path, filename: str = "tiny_dit.safetensors") -> str:
    """Write a tiny safetensors file to stand in for a Qwen-Image DiT.
...
class _TinyQwenImageStub(torch.nn.Module):
    """Stand-in for a Qwen-Image DiT for hotswap-state capture tests.
...
class TestQwenImageHotswapParser(unittest.TestCase):
    """Qwen-Image parser inherits hotswap flags via setup_parser_compile()."""

    def _parse_qwen_image(self, extra_args: list[str]):
    ...
    def test_qwen_image_parse_accepts_hotswap_flags(self) -> None:
        args = self._parse_qwen_image(["--prepare_for_hotswap", "--cache_unmerged_base", "--no-hotswap_strict_base_hash"])
    ...
    def test_qwen_image_parse_default_off(self) -> None:
        args = self._parse_qwen_image([])
    ...
    def test_qwen_image_hotswap_rejects_prefer_lycoris(self) -> None:
        with self.assertRaisesRegex(ValueError, r"prepare_for_hotswap.*prefer_lycoris"):
            self._parse_qwen_image(["--prepare_for_hotswap", "--prefer_lycoris"])

    def test_qwen_image_hotswap_rejects_fp8_scaled(self) -> None:
        with self.assertRaisesRegex(ValueError, r"prepare_for_hotswap.*--fp8_scaled"):
            self._parse_qwen_image(["--prepare_for_hotswap", "--fp8_scaled"])

    def test_qwen_image_hotswap_rejects_fp8(self) -> None:
        # Tighter than `fp8` to avoid matching the fp8_scaled error message.
        with self.assertRaisesRegex(ValueError, r"prepare_for_hotswap.*--fp8 in Qwen-Image"):
            self._parse_qwen_image(["--prepare_for_hotswap", "--fp8"])

    def test_qwen_image_hotswap_rejects_save_merged_model(self) -> None:
        with self.assertRaisesRegex(ValueError, r"prepare_for_hotswap.*save_merged_model"):
            self._parse_qwen_image(["--prepare_for_hotswap", "--save_merged_model", "merged.safetensors"])

    def test_qwen_image_hotswap_rejects_latent_path(self) -> None:
        from musubi_tuner import qwen_image_generate_image
        ...
        with patch.object(sys, "argv", argv):
            with self.assertRaisesRegex(ValueError, r"prepare_for_hotswap.*latent_path"):
                qwen_image_generate_image.parse_args()
...
class TestPrepareQwenImageHotswapState(unittest.TestCase):
    """prepare_qwen_image_hotswap_state() shape contract."""
    ...
    def test_off_sets_hotswap_state_to_none(self) -> None:
        from musubi_tuner.qwen_image_generate_image import prepare_qwen_image_hotswap_state

        model = _TinyQwenImageStub()
        result = prepare_qwen_image_hotswap_state(model, self._args(prepare_for_hotswap=False))
    ...
    def test_on_with_cache_in_ram_captures_state(self) -> None:
        ...
        result = prepare_qwen_image_hotswap_state(model, self._args(prepare_for_hotswap=True, cache_unmerged_base=True))
    ...
    def test_on_with_reload_mode_stores_only_paths(self) -> None:
        ...
        model = _TinyQwenImageStub()
        result = prepare_qwen_image_hotswap_state(model, self._args(prepare_for_hotswap=True, cache_unmerged_base=False))
    ...
    def test_strict_base_hash_threaded_through(self) -> None:
        ...
        model = _TinyQwenImageStub()
        result = prepare_qwen_image_hotswap_state(
            model,
...
class TestNoteQwenImageInitialLoras(unittest.TestCase):
    """note_qwen_image_initial_loras() bookkeeping shape."""
    ...
    def _model_with_state(self, args: argparse.Namespace) -> _TinyQwenImageStub:
        from musubi_tuner.qwen_image_generate_image import prepare_qwen_image_hotswap_state
    ...
    def test_off_is_noop(self) -> None:
        from musubi_tuner.qwen_image_generate_image import note_qwen_image_initial_loras
...
class TestQwenImageLoadDitModelLifecycleOrdering(unittest.TestCase):
    ...
    positions of the four hotswap-relevant statements. Pinned against future
    refactors that might silently reorder the lifecycle.
    """

    def _load_dit_model_lines(self) -> list[str]:
        import musubi_tuner.qwen_image_generate_image as mod
    ...
    def test_capture_after_load_before_optimize(self) -> None:
        lines = self._load_dit_model_lines()
        load_idx = next(i for i, ln in enumerate(lines) if "qwen_image_model.load_qwen_image_model" in ln)
        capture_idx = next(i for i, ln in enumerate(lines) if "prepare_qwen_image_hotswap_state(model, args)" in ln)
        # Block-swap is the canonical "first optimize step" for Qwen-Image
    ...
    def test_initial_merge_uses_standard_lora_only_under_hotswap(self) -> None:
        ...
        # And the merge must reference lora_qwen_image as the network module
        self.assertIn("merge_lora_weights(\n            lora_qwen_image,", joined)
    ...
    def test_note_after_initial_merge(self) -> None:
        ...
        note_idx = next(i for i, ln in enumerate(lines) if "note_qwen_image_initial_loras(model, args)" in ln)
        self.assertLess(merge_idx, note_idx)


class TestQwenImageLoadDitModelMockedIntegration(unittest.TestCase):
    ...
    def test_load_branch_suppresses_preload_then_captures_merges_and_notes(self) -> None:
        ...
        model = _TinyQwenImageStub()
        events: list[str] = []
        original_prepare = mod.prepare_qwen_image_hotswap_state
        original_note = mod.note_qwen_image_initial_loras
        ...
        with (
            patch.object(mod.qwen_image_model, "load_qwen_image_model", side_effect=load_spy),
            patch.object(mod, "prepare_qwen_image_hotswap_state", side_effect=prepare_spy),
            patch.object(mod, "merge_lora_weights", side_effect=merge_spy),
            patch.object(mod, "note_qwen_image_initial_loras", side_effect=note_spy),
